    ...
    <div class="container-fluid">
        ...
        <nav class="py-4">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('PaymentMethod') }}</li>
            </ol>
        </nav>
        <div class="row">
            <div class="col-xl-12">
                    ...
                    <div class="card-header justify-content-between">
                        ...
                        <div class="d-flex align-items-center justify-content-center gap-2">
                            <button type="button" name="create_record" id="create_record" class="btn btn-success btn-sm">
                                {{ __('Create PaymentMethod') }}</button>
                                <form id="delete_form" action="{{ route('common.deleteSelected', 'PaymentMethod') }}" class="mb-0"
                                method="POST">
                                @csrf
...
@section('scripts')
    ...
    <script>
        ...
        $('#paymentmethod-table').on('change', '.row-select', function() {
            let id = $(this).val();
            if (this.checked) {
                selectedIds.add(id);
            } else {
        ...
        $(document).ready(function() {
            ...
            $('#ok_btn').on('click', function() {
                var currentPage = window.LaravelDataTables["paymentmethod-table"].page.info().page + 1;
            ...
            $('#paymentmethod-table').on('init.dt', function() {
                // Get the DataTable instance once initialized
                var table = window.LaravelDataTables["paymentmethod-table"];
                ...
                if (table) {
                    ...
                    console.error("DataTable 'paymentmethod-table' is not initialized.");
                }
            });
        });
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#create_record').click(function() {
                $('.modal-title').text('{{ __('Add New PaymentMethod') }}');
            ...
            $('#sample_form').on('submit', function(event) {
                ...
                if ($('#action').val() == 'Add') {
                    action_url = "{{ route('payment-method.store') }}";
                    $.ajax({
                        url: action_url,
                        method: "POST",
                        data: formdata,
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                html = '<div class="alert alert-success">' + data.message +
                                    '</div>';
                                $('#sample_form')[0].reset();
                                window.LaravelDataTables["paymentmethod-table"].ajax.reload();
                                setTimeout(function() {
                ...
                if ($('#action').val() == 'Edit') {
                    ...
                    action_url = "{{ url('admin/payment-method') }}" + "/" + dataId;
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                ...
                                var currentPage = window.LaravelDataTables["paymentmethod-table"]
                                    .page.info()
                                    .page;
                                setTimeout(function() {
                                    $('#formModal').modal('hide'); // Hide the modal
                                }, 1000);
                                window.LaravelDataTables["paymentmethod-table"].ajax.reload(function(
                                    json) {
                                    window.LaravelDataTables["paymentmethod-table"].page(
                                            currentPage)
                                        .draw(false);
                                }, false);
                            }
            ...
            $('#ok_button').click(function() {
                $.ajax({
                    ...
                    success: function(data) {
                        // Get the current page number of the DataTable
                        var currentPage = window.LaravelDataTables["paymentmethod-table"].page.info()
                            .page;
                        setTimeout(function() {
                            $('#confirmModal').modal('hide');
                        }, 2000);
                        window.LaravelDataTables["paymentmethod-table"].ajax.reload(function(json) {
                            window.LaravelDataTables["paymentmethod-table"].page(currentPage)
                                .draw(false);
                        }, false);
                    }
                })
