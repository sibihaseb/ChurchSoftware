    ...
    <div class="container-fluid">
        ...
        <nav class="py-4">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('MemberType') }}</li>
            </ol>
        </nav>
        <div class="row">
            <div class="col-xl-12">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#create_record').click(function() {
                $('.modal-title').text('{{ __('Add New MemberType') }}');
            ...
            $('#sample_form').on('submit', function(event) {
                ...
                if ($('#action').val() == 'Add') {
                    action_url = "{{ route('member-type.store') }}";
                    $.ajax({
                        url: action_url,
                        method: "POST",
                        data: formdata,
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                html = '<div class="alert alert-success">' + data.message +
                                    '</div>';
                                $('#sample_form')[0].reset();
                                window.LaravelDataTables["membertype-table"].ajax.reload();
                                setTimeout(function() {
                ...
                if ($('#action').val() == 'Edit') {
                    ...
                    action_url = "{{ url('admin/member-type') }}" + "/" + dataId;
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                ...
                                var currentPage = window.LaravelDataTables["membertype-table"]
                                    .page.info()
                                    .page;
                                setTimeout(function() {
                                    $('#formModal').modal('hide'); // Hide the modal
                                }, 1000);
                                window.LaravelDataTables["membertype-table"].ajax.reload(function(
                                    json) {
                                    window.LaravelDataTables["membertype-table"].page(
                                            currentPage)
                                        .draw(false);
                                }, false);
                            }
            ...
            $('#ok_button').click(function() {
                $.ajax({
                    ...
                    success: function(data) {
                        // Get the current page number of the DataTable
                        var currentPage = window.LaravelDataTables["membertype-table"].page.info()
                            .page;
                        setTimeout(function() {
                            $('#confirmModal').modal('hide');
                        }, 2000);
                        window.LaravelDataTables["membertype-table"].ajax.reload(function(json) {
                            window.LaravelDataTables["membertype-table"].page(currentPage)
                                .draw(false);
                        }, false);
                    }
                })
