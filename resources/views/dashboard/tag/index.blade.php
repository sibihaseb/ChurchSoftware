                    ...
                    <div class="card-header justify-content-between">
                        ...
                        <div class="d-flex align-items-center justify-content-center gap-2">
                            <button type="button" name="create_record" id="create_record" class="btn btn-success btn-sm">
                                ...
                                <form id="delete_form" action="{{ route('common.deleteSelected', 'Tag') }}" class="mb-0"
                                method="POST">
                                @csrf
...
@section('scripts')
    ...
    <script>
        ...
        $('#tag-table').on('change', '.row-select', function() {
            let id = $(this).val();
            if (this.checked) {
                selectedIds.add(id);
            } else {
        ...
        $(document).ready(function() {
            ...
            $('#ok_btn').on('click', function() {
                var currentPage = window.LaravelDataTables["tag-table"].page.info().page + 1;
            ...
            $('#tag-table').on('init.dt', function() {
                // Get the DataTable instance once initialized
                var table = window.LaravelDataTables["tag-table"];
                ...
                if (table) {
                    ...
                    console.error("DataTable 'tag-table' is not initialized.");
                }
            });
        });
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#create_record').click(function() {
                $('.modal-title').text('{{ __('Add New Tag') }}');
            ...
            $('#sample_form').on('submit', function(event) {
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                html = '<div class="alert alert-success">' + data.message +
                                    '</div>';
                                $('#sample_form')[0].reset();
                                window.LaravelDataTables["tag-table"].ajax.reload();
                                setTimeout(function() {
                ...
                if ($('#action').val() == 'Edit') {
                    ...
                    action_url = "{{ url('admin/tag') }}" + "/" + dataId;
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                ...
                                var currentPage = window.LaravelDataTables["tag-table"]
                                    .page.info()
                                    .page;
                                setTimeout(function() {
                                    $('#formModal').modal('hide'); // Hide the modal
                                }, 1000);
                                window.LaravelDataTables["tag-table"].ajax.reload(function(
                                    json) {
                                    window.LaravelDataTables["tag-table"].page(
                                            currentPage)
                                        .draw(false);
                                }, false);
                            }
            ...
            $('#ok_button').click(function() {
                $.ajax({
                    ...
                    success: function(data) {
                        // Get the current page number of the DataTable
                        var currentPage = window.LaravelDataTables["tag-table"].page.info()
                            .page;
                        setTimeout(function() {
                            $('#confirmModal').modal('hide');
                        }, 2000);
                        window.LaravelDataTables["tag-table"].ajax.reload(function(json) {
                            window.LaravelDataTables["tag-table"].page(currentPage)
                                .draw(false);
                        }, false);
                    }
                })
