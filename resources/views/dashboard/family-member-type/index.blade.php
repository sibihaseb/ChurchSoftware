    ...
    <div class="container-fluid">
        ...
        <nav class="py-4">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('FamilyMemberType') }}</li>
            </ol>
        </nav>
        <div class="row">
            <div class="col-xl-12">
                    ...
                    <div class="card-header justify-content-between">
                        ...
                        <div class="d-flex align-items-center justify-content-center gap-2">
                            <button type="button" name="create_record" id="create_record" class="btn btn-success btn-sm">
                                {{ __('Create FamilyMemberType') }}</button>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#create_record').click(function() {
                $('.modal-title').text('{{ __('Add New FamilyMemberType') }}');
            ...
            $('#sample_form').on('submit', function(event) {
                ...
                if ($('#action').val() == 'Add') {
                    action_url = "{{ route('family-member-type.store') }}";
                    $.ajax({
                        url: action_url,
                        method: "POST",
                        data: formdata,
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                html = '<div class="alert alert-success">' + data.message +
                                    '</div>';
                                $('#sample_form')[0].reset();
                                window.LaravelDataTables["familymembertype-table"].ajax.reload();
                                setTimeout(function() {
                ...
                if ($('#action').val() == 'Edit') {
                    ...
                    action_url = "{{ url('admin/family-member-type') }}" + "/" + dataId;
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                ...
                                var currentPage = window.LaravelDataTables["familymembertype-table"]
                                    .page.info()
                                    .page;
                                setTimeout(function() {
                                    $('#formModal').modal('hide'); // Hide the modal
                                }, 1000);
                                window.LaravelDataTables["familymembertype-table"].ajax.reload(function(
                                    json) {
                                    window.LaravelDataTables["familymembertype-table"].page(
                                            currentPage)
                                        .draw(false);
                                }, false);
                            }
            ...
            $('#ok_button').click(function() {
                $.ajax({
                    url: "family-member-type/" + user_id,
                    type: "DELETE",
                    headers: {
                    ...
                    success: function(data) {
                        // Get the current page number of the DataTable
                        var currentPage = window.LaravelDataTables["familymembertype-table"].page.info()
                            .page;
                        setTimeout(function() {
                            $('#confirmModal').modal('hide');
                        }, 2000);
                        window.LaravelDataTables["familymembertype-table"].ajax.reload(function(json) {
                            window.LaravelDataTables["familymembertype-table"].page(currentPage)
                                .draw(false);
                        }, false);
                    }
                })
