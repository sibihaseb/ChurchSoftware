                    ...
                    <div class="card-header justify-content-between">
                        ...
                        <div class="d-flex align-items-center justify-content-center gap-2">
                            <button type="button" name="create_record" id="create_record" class="btn btn-success btn-sm">
                                ...
                                <form id="delete_form" action="{{ route('common.deleteSelected', 'Product') }}" class="mb-0"
                                method="POST">
                                @csrf
...
@section('scripts')
    ...
    <script>
        ...
        $('#product-table').on('change', '.row-select', function() {
            let id = $(this).val();
            if (this.checked) {
                selectedIds.add(id);
            } else {
        ...
        $(document).ready(function() {
            ...
            $('#ok_btn').on('click', function() {
                var currentPage = window.LaravelDataTables["product-table"].page.info().page + 1;
            ...
            $('#product-table').on('init.dt', function() {
                ...
                if (table) {
                    ...
                    console.error("DataTable 'product-table' is not initialized.");
                }
            });
        });
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#create_record').click(function() {
                $('.modal-title').text('{{ __('Add New Product') }}');
            ...
            $('#sample_form').on('submit', function(event) {
                ...
                if ($('#action').val() == 'Edit') {
                    ...
                    action_url = "{{ url('admin/product') }}" + "/" + dataId;
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.message) {
                                ...
                                var currentPage = window.LaravelDataTables["product-table"]
                                    .page.info()
                                    .page;
                                setTimeout(function() {
                                    $('#formModal').modal('hide'); // Hide the modal
                                }, 1000);
                                window.LaravelDataTables["product-table"].ajax.reload(function(
                                    json) {
                                    window.LaravelDataTables["product-table"].page(
                                            currentPage)
                                        .draw(false);
                                }, false);
                            }
            ...
            $('#ok_button').click(function() {
                $.ajax({
                    ...
                    success: function(data) {
                        // Get the current page number of the DataTable
                        var currentPage = window.LaravelDataTables["product-table"].page.info()
                            .page;
                        setTimeout(function() {
                            $('#confirmModal').modal('hide');
                        }, 2000);
                        window.LaravelDataTables["product-table"].ajax.reload(function(json) {
                            window.LaravelDataTables["product-table"].page(currentPage)
                                .draw(false);
                        }, false);
                    }
                })
